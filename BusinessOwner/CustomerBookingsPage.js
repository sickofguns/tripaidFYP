  ...
  const fetchBookingsFromDatabase = async () => {
    try {
      ...
      const bookingsData = snapshot.docs.map(doc => {
        const data = doc.data();
        // Extract the date from 'date', 'startdate', or 'checkin'
        let date = data.date || data.startdate || data.checkin;
  
        // Format the date to "February 19, 2024"
        const formattedDate = date ? formatDate(date.toDate()) : '';
  
        // Format the time to "12:16 PM"
        const formattedTime = date ? formatTime(date.toDate()) : '';
  
        const bookingRef = data.userId;
        const time = formattedTime || ''; // Use the formatted time or an empty string if not available
        return { date: formattedDate, bookingRef, time };
      });
      return bookingsData;
    } catch (error) {
