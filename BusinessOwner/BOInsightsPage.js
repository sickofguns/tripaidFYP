...
const calculateTotalUserCount = async (startDate, endDate) => {
  try {
    ...
    const uniqueUsers = boUsers.filter(boUser => !lolUsers.some(lolUser => lolUser.businessUserId === boUser.businessUserId));
...
if (lastWeekUserCount !== 0) {
  ...
  percentageIncrease = thisWeekUserCount * 100;
    ...
    const [newBookingData, setNewBookingData] = useState(Array(7).fill(0));
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          const snapshot = await getDocs(query(usersCollection, where('businessname', '==', user.username), where('businessId', '==', user.id)));
  
          const newBookingDataArray = Array(7).fill(0);
          ...
          snapshot.docs.forEach((doc) => {
            ...
            if (create) {
              const dayOfWeek = new Date(create.toDate()).getDay();
              newBookingDataArray[dayOfWeek]++;
              newBookingCount++;
            }
          });
  
          setNewBookingData(newBookingDataArray);
          setNewBookingCount(newBookingCount);
        } catch (error) {
      ...
      const interval = setInterval(() => {
        setNewBookingData(Array(7).fill(0));
    ...
    const chartData = newBookingData.map((value, index) => ({ date: daysOfWeek[index], value }));
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContainer}>
                ...
                {/* Usage Time Section */}
                <View style={styles.insightsSection}>
                <Text style={styles.sectionTitle}>Booking Demands</Text>
                <Text style={styles.chartTitle}>Demands Reached in the Past 7 Days</Text>
                {/* Add your content related to account engagement here */}
                {/* For example, text, charts, or any other relevant information */}
                <LineGraph data={chartData} />

                <Text style={styles.perDayText}>Total Bookings This Week</Text>
                <Text style={styles.averageTimeText}>{newBookingCount} Bookings</Text>
...
export default BOInsightsScreen;