  ...
  const fetchUsersFromDatabase = async (inputQuery) => {
    try {
      ...
      if (inputQuery) {
        ...
        userQuery = query(
          usersCollection,
          where('lolUsername', '==', inputQuery),
          where('collab', '==', true),
          where('businessUserId', '==', user.id) // Add this line to filter by businessUserId
        );
      } else {
...
const [BOCount, setBOCount] = useState();
  ...
  const handleDeleteUser = (userId, lolUsername, collab) => {
    Alert.alert(
      `Confirm Remove User ${lolUsername} from Collaboration?`,
      '',
      [
        {
          text: 'No',
        ...
        {
          text: 'Yes',
          onPress: () => deleteUser(userId, lolUsername, collab),
          style: 'destructive',
        },
      ],
      { cancelable: false }
  ...
  const renderUserItem = ({ item }) => (
    <View style={styles.userItem}>
      <TouchableOpacity onPress={() => navigateToUserProfile(item.lolUsername)}>
        <View style={styles.userInfo}>
          <Text style={styles.usernamecontent}>{item.lolUsername}</Text>
          {/* Add other user details you want to display */}
        </View>
      </TouchableOpacity>
      <View style={styles.userIcons}>
        <TouchableOpacity onPress={() => handleDeleteUser(item.id, item.lolUsername)}>
  ...
  const handleUpgradeUser = (userId, lolUsername) => {
    Alert.alert(
      `Confirm Collaborate with User ${lolUsername} ?`,
      '',
      [
        {
          text: 'No',
  ...
  const handleDenyUser = (userId, lolUsername) => {
    Alert.alert(
      `Confirm Deny Collaboration User ${lolUsername} ?`,
      '',
      [
        {
          text: 'No',
  ...
  const confirmUpgrade = async (userId) => {
    try {
      ...
      if (userToUpgrade) {
        ...
        // Update the user's affiliation status in the database
        await updateDoc(affiliationDocRef, { collab: true, pending: false, reached: true });
        ...
        Alert.alert('User Confirmed', `User ${userToUpgrade.lolUsername} collaboration successfully.`);
      } else {
  ...
  const denyUpgrade = async (userId) => {
    try {
      ...
      if (userToDeny) {
        const affiliationDocRef = doc(db, 'lolaffiliations', userId); // Assuming the document ID in the lolaffiliations collection is the same as the user ID
        ...
        Alert.alert('User Denied', `User ${userToDeny.lolUsername} collaboration failed.`);
      } else {
  ...
  const renderPendingItem = ({ item }) => (
    <View style={styles.pendingItem}>
      ...
      <View style={styles.pendingIcons}>
        <TouchableOpacity onPress={() => handleUpgradeUser(item.id, item.lolUsername)}>
          <MaterialIcons name="check" size={20} color="green" style={[styles.icon, styles.tickIcon]} />
        </TouchableOpacity>
        <TouchableOpacity onPress={() => handleDenyUser(item.id, item.lolUsername)}>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.midContainer}>
        ...
        <Text style={styles.secondaryText}>LOL Collaboration Accounts</Text>
