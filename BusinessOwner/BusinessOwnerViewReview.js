...
const BOSpecificReviewScreen = ({ route }) => {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////edit 
    ...
    const handleReply = async () => {
      try {
        ...
        const userReply = await new Promise((resolve, reject) => {
          Alert.prompt(
            'Reply to Review',
        ...
        if (userReply) {
          ...
          const reviewRef = doc(collection(db, 'reviews'), review.id);
          await updateDoc(reviewRef, { reply: userReply });
    
          // Update the local state with the reply
          setReply(userReply);
        }
      } catch (error) {
    ...
    const handleChangeReply = async () => {
      try {
        ...
        const modifiedReply = await new Promise((resolve, reject) => {
          Alert.prompt(
            'Change Reply',
        ...
        if (modifiedReply) {
          // Update the modified reply in Firebase
          const reviewRef = doc(collection(db, 'reviews'), review.id);
          await updateDoc(reviewRef, { reply: modifiedReply });
    
          // Update the local state with the modified reply
          setReply(modifiedReply);
        }
      } catch (error) {
        console.error('Error changing reply:', error);
        // Handle the error as needed
      }
    };
