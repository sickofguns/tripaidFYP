...
import { collection, updateDoc, doc } from "firebase/firestore/lite";
import { useAppContext } from "../AppContext";
import DateTimePicker from "@react-native-community/datetimepicker";

//EDIIT PAGE
const BOEditListingScreen = ({ route }) => {
  ...
  const handleupdate = async () => {
    ...
    try {
      if (!promotion || !title || !thumbnail || !description || !valid) {
        alert("Please fill in all required fields (Title, Image, Valid, Promotion Details and Code) before updating.");
        console.log (valid);
        return;
      }
      // Update the review in the database
      await updateDoc(doc(db, 'promos', promo.id), updatedPromo);

      // Show alert on publishing review
    ...
    <View style={styles.container}>
      ...
      <View style={styles.midContainer}>
        ...
        <TouchableOpacity style={styles.createButton} onPress={handleupdate}>
          <Text style={styles.createButtonText}>Update Promotion</Text>
        </TouchableOpacity>
...
export default BOEditListingScreen;
