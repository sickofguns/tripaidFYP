...
const BOSpecificBookingScreen = () => {
  ...
  useEffect(() => {
    ...
    console.log("Booking Data:", booking); // Add this line
    ...
    <View style={styles.container}>
      ...
      <View style={styles.midContainer}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          {booking && (
            <View>
              {booking.category.toLowerCase() === "accommodation" && (
                <View>
                  <Text style={styles.categoryText}>Accommodation Reservation at {booking.location}</Text>
                  <Text style={styles.detailText}>Customer Name: {booking.username}</Text>
                  <Text style={styles.detailText}>Check-in date: {(booking.date)}</Text>
                  <Text style={styles.detailText}>Check-out date: {(booking.checkoutdate)}</Text>
                  <Text style={styles.detailText}>Location: {booking.location}</Text>
                  <Text style={styles.detailText}>Pax: Adults: {booking.adults} Children: {booking.children}</Text>   
                  <Text style={styles.detailText}>Promo: {booking.promo}</Text>                
                  <Text style={styles.detailText}>Remarks: {booking.remark}</Text>          
                </View>
              )}
              {["food", "attraction", "wellness", "tours", "events"].includes(booking.category.toLowerCase()) && (
                <View>
                  <Text style={styles.categoryText}>{booking.category.charAt(0).toUpperCase() + booking.category.slice(1)} Reservation at {booking.location}</Text>
                  <Text style={styles.detailText}>Customer Name: {booking.username}</Text>
              ...
              {booking.category.toLowerCase() === "transport" && (
                <View>
                  <Text style={styles.categoryText}>Transport Rental at {booking.location}</Text>
                  <Text style={styles.detailText}>Customer Name: {booking.username}</Text>
                  <Text style={styles.detailText}>Start date: {(booking.date)}</Text>
                  <Text style={styles.detailText}>End date: {(booking.enddate)}</Text>
                  <Text style={styles.detailText}>Time: {(booking.time)}</Text>
                  <Text style={styles.detailText}>Location: {booking.location}</Text>
                  <Text style={styles.detailText}>Pax: Adults: {booking.adults} Children: {booking.children}</Text>       
                  <Text style={styles.detailText}>Promo: {booking.promo}</Text>                   
...
export default BOSpecificBookingScreen;
