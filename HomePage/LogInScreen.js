...
const LogInScreen = () => {
  ...
  const handleResetPassword = () => {
    ...
    // Close the modal after a brief delay and reset state
  setTimeout(() => {
    setModalVisible(false);
    setNewPassword(''); // Reset newPassword state
    setConfirmNewPassword(''); // Reset confirmNewPassword state
    setErrorText(''); // Clear the error message
  }, 1600); // Change the duration (in milliseconds) as needed
  };
