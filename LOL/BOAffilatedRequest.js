...
const LOLAffiliateBOScreen = () => {
  ...
  const fetchAffiliationStatus = async (businessUserId) => {
    try {
      const affiliationsCollection = collection(db, "lolaffiliations");
  ...
  const handleSearching = async () => {
    try {
      ...
      console.error("Error searching business users:", error);
    }
  };

  const handleReachOutToggle = async (businessUserId) => {
    try {
      // Fetch usernames for business and LOL accounts
      ...
      if (affiliationsDocs.length > 0) {
        // If there are existing lolaffiliations, update each one
        ...
        // If there are no existing lolaffiliations, create a new one
        await addDoc(affiliationsCollection, {
          businessUserId,
          businessUsername: businessUsername.username,
          lolUsername: lolUsername.username,
  ...
  useEffect(() => {
    // Load all business users from the database initially
...
export default LOLAffiliateBOScreen;
