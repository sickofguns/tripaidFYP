...
const LOLBookingHistoryScreen = () => {
  ...
  return () => {
    navigation.navigate("LOL View Booking", { bookingData: item, documentId: item.documentId });
  };
};

  const handleViewHistoryContent = (item) => {
    return () => {
    navigation.navigate("LOL View History Booking", { bookingData: item, documentId: item.documentId });
