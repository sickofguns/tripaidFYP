...
const LOLEditTrailScreen = ({ route }) => {
  ...
  useEffect(() => {
    ...
    const handlepublish = async () => {
      // Extract trail details from the form
      const updatedTrail = {
        location: place,
        category: category,
        title: title,
      ...
      try {
        if (!place || place === 'Select a location' || !category || category === 'Select a category' || !title || !thumbnail || !description) {
          alert("Please fill in all required fields (Location, POI Category, Title, Image and Caption) before updating.");
          return;
        }
        // Update the trail in the database
        await updateDoc(doc(db, 'trails', trail.id), updatedTrail);
  
        // Show alert on publishing trail
        alert('Trail updated!');
        navigation.goBack();
      } catch (error) {
        ...
        <View style={styles.container}>
          ...
          <View style={styles.titleContainer}>
            ...
            <View style={styles.Detailscontainer}>
                ...
                <Text style={styles.createButtonText}>Update Trail</Text>
                </TouchableOpacity>
...
export default LOLEditTrailScreen;