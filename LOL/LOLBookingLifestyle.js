...
const LOLLifestyleBookingScreen = () => {
  const { user } = useAppContext();
  const [currentLocation, setCurrentLocation] = useState("Loading..."); // Initialize with a loading message
  const [location, setLocation] = useState("");
  const [lifestyles, setLifestyles] = useState([]);
  const [lifestyleLocations, setLifestyleLocations] = useState({});
  const [selectedLifestyleError, setSelectedLifestyleError] = useState(false);
  ...
  useEffect(() => {
    ...
    fetchLocation();
      ...
      // Fetch lifestyles from Firestore
      const fetchLifestyles = async () => {
        try {
          const lifestylesRef = collection(db, "users");
          const querySnapshot = await getDocs(lifestylesRef);
  
          const fetchedLifestyles = [];
          const fetchedLifestyleLocations = {};
  
          querySnapshot.forEach((doc) => {
            const userData = doc.data();
            if (userData.category === "Wellness") {
              fetchedLifestyles.push({ label: userData.username, value: userData.username });
              fetchedLifestyleLocations[userData.username] = userData.address;
            }
          });
  
          setLifestyles(fetchedLifestyles);
          setLifestyleLocations(fetchedLifestyleLocations);
        } catch (error) {
  ...
  const handleLifestyleChange = (itemValue) => {
    setSelectedLifestyle(itemValue);
    setPlace(lifestyleLocations[itemValue]);
  };


  const [selectedLifestyle, setSelectedLifestyle] =
    useState("Select Lifestyle");
  ...
  const handlebooking = async () => {
    ...
    if (!selectedLifestyle || !place || !checkindate || !checkoutdate || !adults ) {
      ...
      try {
        ...
        userQuerySnapshot.forEach((doc) => {
          ...
          if (userData.username === selectedLifestyle) {
        ...
        const bookData = {
          category: 'Wellness',
          businessname: selectedLifestyle,
        ...
        navigation.navigate("LOL Booking Confirmation Page");
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.midContainer}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          <View style={styles.titleContainer}>
            ...
            <View style={styles.inputContainer}>
              <Picker
                selectedValue={selectedLifestyle}
                onValueChange={handleLifestyleChange}
                style={styles.dropdownBox}
                itemStyle={{ fontSize: 16, width: 350 }}
              >
                <Picker.Item label="Select lifestyle" value="Lifestyle" />
        ...
        {(selectedLifestyleError || placeError) && (
...
export default LOLLifestyleBookingScreen;
