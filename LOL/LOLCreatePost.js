...
const LOLNewPostScreen = () => {
  ...
  const handlepublish = async () => {
    try {
      ...
      if (!place || place === 'Select a location' || !category || category === 'Select a category' || !thumbnail || !description) {
        alert("Please fill in all required fields (Location, POI Category, Image and Caption) before publishing.");
        return;
      }
  ...
  const handleImageSelection = async () => {
    ...
    if (selectedImage) {
      setThumbnail(selectedImage.uri);
    }
  };
