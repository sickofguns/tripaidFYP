...
import { collection, deleteDoc, doc, addDoc, getDoc, updateDoc, where, query, getDocs } from "firebase/firestore/lite";
import { useAppContext } from "../AppContext";

const LOLSpecificItineraryScreen = ({ route }) => {
  ...
  const handleedit = () => {
    navigation.navigate("LOL Edit Itinerary", { itinerary }); // Navigate to SignUp screen on button press
  };
...
// Function to update the database and set trail status to true
const updateDatabaseForTrail = async () => {
  try {
    // Reference to the Firestore collection for trails
    ...
    // Fetch the specific itinerary data from the database
    const itinerarySnapshot = await getDoc(itineraryDocRef);
    const itineraryData = itinerarySnapshot.data();

    // Check if 'itineraryData' exists and has the required properties
    if (
      itineraryData &&
      itineraryData.location &&
      itineraryData.category &&
      itineraryData.title &&
      itineraryData.thumbnail &&
      itineraryData.description &&
      itineraryData.userId &&
      itineraryData.username &&
      itineraryData.created
    ) {

      // Add a new document to the trails collection with the fetched itinerary data
      const trailData = {
        ...
        created: itineraryData.created,
        trail: true,
        likes: 0,
        // Add other fields you want to store in the trails collection
      };

      const newTrailDocRef = await addDoc(trailsCollectionRef, trailData);

      // Log the ID of the newly added trail document
      console.log("New Trail Document ID:", newTrailDocRef.id);

      // Update the existing itinerary in the itineraries collection to mark it as a trail
      await updateDoc(itineraryDocRef, {
        trail: true,
      });
    } else {
      throw new Error("Invalid itinerary data. Missing required properties.");
    }
  } catch (error) {
    console.error("Error updating database for trail:", error);
  }
};
...
const handlepublish = async () => {
  try {
    // Fetch the specific itinerary data from the database
    const itinerarySnapshot = await getDoc(
      doc(collection(db, "itineraries"), itinerary.id)
    );
    const itineraryData = itinerarySnapshot.data();

    // Check if the itinerary is already marked as a trail
    if (itineraryData && itineraryData.trail) {
      // Show alert for itinerary already marked as a trail
      Alert.alert("Error", "Itinerary has already been posted as Trail.");
    } else {
...
// Function to handle the success message after publishing as trail
      ...
      <View style={styles.midContainer}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          <View>
            ...
            {/* Image */}
            <Image source={itinerary.imageUrl} style={styles.image} />
            ...
            <View style={styles.contentContainer}>
              <Text style={styles.subText}>{itinerary.description}</Text>
            </View>
          </View>
        </ScrollView>
