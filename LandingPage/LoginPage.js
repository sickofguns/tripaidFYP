...
const LoginPageScreen = () => {
  ...
  const handleLogin = async () => {
    ...
    if (userQuerySnapshot.empty) {
      setErrorText("Invalid email or username"); // Show error if no matching user found
      setUsernameError(true); // Set error state to true for invalid email or username
      return;
    }

    // Assuming there is only one user with a given email or username
    const userData = userQuerySnapshot.docs[0].data();

    if (userData.password.toLowerCase() !== password.toLowerCase()) {
