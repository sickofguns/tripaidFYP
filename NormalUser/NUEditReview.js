...
const NUEditReviewScreen = ({ route }) => {
  ...
  useEffect(() => {
   ...
   const [category, setCategory] = useState('');
    ...
    const handlepublish = async () => {
      ...
      // Fetch the business user ID
    const businessname = await fetchUSERNAMEByCategoryandID(category, business);

    // Validate if the business user ID is fetched
    if (businessname === undefined) {
      alert("Error fetching business user ID. Please try again.");
      return;
    }
      ...
      const updatedReview = {
        category: category,
        business: businessname, // Use the business name
        businessUserId: business, // Use the business user ID        
        title: title,
        location: place,
        thumbnail: thumbnail,
      ...
      try {
        if (!place || place === 'Select a location' || !category || !business || business === 'No business found' || business === '' || !title || !thumbnail || !description) {
          alert("Please fill in all required fields (Category, Business, Location, Title, Thumbnail and Details) before updating.");
          return;
        }
        // Update the review in the database
        await updateDoc(doc(db, 'reviews', review.id), updatedReview);
...
export default NUEditReviewScreen;