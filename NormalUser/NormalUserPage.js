        ...
        <View style={styles.postsContainer}>
          {posts.map((post, index) => (
            <View key={post.id} style={styles.instagramPostContainer}>
              <Image style={styles.postImage} source={{ uri: String(post.imageUrl) }} />
...
// Rest of your component code
  ...
  const handleCollect = async (promoId, dealName) => {
    ...
    if (user && user.id) {
      ...
      try {
        ...
        // Save the collected state to AsyncStorage
        await AsyncStorage.setItem(`collected_${promoId}`, 'collected');
  ...
  const handleRedeem = async (promoId, dealName) => {
    ...
    if (user && user.id) {
      ...
      try {
        ...
        // Save the redeemed state to AsyncStorage
        await AsyncStorage.setItem(`redeemed_${promoId}`, 'redeemed');
  ...
  useEffect(() => {
    const loadRedeemedStates = async () => {
      try {
        // Load the collected and redeemed states from AsyncStorage
        const storedPromos = await AsyncStorage.multiGet(
          promos.map((promo) => `collected_${promo.id}`)
        );
  
        const redeemedStates = {};
        storedPromos.forEach(([key, value]) => {
          if (value) {
            redeemedStates[key.replace('collected_', '')] = value;
          }
        });
  
        setRedeemedStates(redeemedStates);
      } catch (error) {
        console.error("Error loading redeemed states:", error);
      }
    };
