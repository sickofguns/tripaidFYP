...
const NUAttractionBookingScreen = () => {
  ...
  const [selectedAttractionError, setSelectedAttractionError] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchAttractions = async () => {
      try {
        const attractionsRef = collection(db, "users");
        const querySnapshot = await getDocs(attractionsRef);

        const fetchedAttractions = [];
        const fetchedAttractionLocations = {};

        querySnapshot.forEach((doc) => {
          const userData = doc.data();
          if (userData.category === "Attraction") {
            fetchedAttractions.push({ label: userData.username, value: userData.username });
            fetchedAttractionLocations[userData.username] = userData.address;
          }
        });

        setAttractions(fetchedAttractions);
        setAttractionLocations(fetchedAttractionLocations);
      } catch (error) {
        console.error("Error fetching attractions:", error);
      }
    };

    // Call the fetchAttractions function
    fetchAttractions();
  ...
  const handleAttractionChange = (itemValue) => {
    setSelectedAttraction(itemValue);
    setPlace(attractionLocations[itemValue]);
  };

  const [selectedAttraction, setSelectedAttraction] =
    useState("Select Attraction");
  ...
  const handlebooking = async () => {
    ...
    if (!selectedAttraction || !place || !checkindate || !checkoutdate || !adults || !rooms) {
      // If any required field is empty, set error flags
      setSelectedAttractionError(!selectedAttraction);
      setPlaceError(!place);
      ...
      <View style={styles.midContainer}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          <View style={styles.titleContainer}>
            <Text style={styles.titleText}>Attraction</Text>
            <View style={styles.inputContainer}>
              ...
              >
                <Picker.Item label="Select Attraction" value="Attraction" />
        ...
        {(selectedAttractionError || placeError) && (
...
export default NUAttractionBookingScreen;
