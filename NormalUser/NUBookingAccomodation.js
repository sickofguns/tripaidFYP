...
const NUAccomodationBookingScreen = () => {
  ...
  const [selectedAccommodationError, setSelectedAccommodationError] = useState(false);
  const [placeError, setPlaceError] = useState(false);
  // Add more error states for other required fields if needed
  ...
  const handlebooking = async () => {
    if (!selectedAccommodation || !place || !checkindate || !checkoutdate || !adults || !rooms) {
      ...
      setSelectedAccommodationError(!selectedAccommodation);
      setPlaceError(!place);
    ...
    if (isValidPromo(promo)) {
      ...
      try {
        ...
        navigation.navigate("Normal User Booking Confirmation Page", { booking: book.id});
      } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.midContainer}>
        ...
        {(selectedAccommodationError || placeError) && (
