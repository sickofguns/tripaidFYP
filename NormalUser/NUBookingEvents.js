...
const NUEventsBookingScreen = () => {
  const { user } = useAppContext();
  const [currentLocation, setCurrentLocation] = useState("Loading..."); // Initialize with a loading message
  const [location, setLocation] = useState("");
  const [events, setEvents] = useState([]);
  const [eventLocations, setEventLocations] = useState({});
  const [selectedEventError, setSelectedEventError] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        const fetchedEvents = [];
        const fetchedEventLocations = {};

        querySnapshot.forEach((doc) => {
          const userData = doc.data();
          if (userData.category === "Events") {
            fetchedEvents.push({ label: userData.username, value: userData.username });
            fetchedEventLocations[userData.username] = userData.address;
          }
        });

        setEvents(fetchedEvents);
        setEventLocations(fetchedEventLocations);
      } catch (error) {
  ...
  const handleEventChange = (itemValue) => {
    setSelectedEvent(itemValue);
    setPlace(eventLocations[itemValue]);
  };
  ...
  const handlebooking = async () => {
    ...
    if (!selectedEvent || !place || !checkindate || !checkoutdate || !adults || !rooms) {
      // If any required field is empty, set error flags
      setSelectedEventError(!selectedEvent);
      setPlaceError(!place);
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          <View style={styles.titleContainer}>
            ...
            <View style={styles.inputContainer}>
              ...
              >
                <Picker.Item label="Select event" value="Event" />
                {events.map((event) => (
                  <Picker.Item
                    label={event.label}
                    value={event.value}
...
export default NUEventsBookingScreen;
