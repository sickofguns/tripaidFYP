...
const NUToursBookingScreen = () => {
  const { user } = useAppContext();
  const [currentLocation, setCurrentLocation] = useState("Loading..."); // Initialize with a loading message
  const [location, setLocation] = useState("");
  const [tours, setTours] = useState([]);
  const [tourLocations, setTourLocations] = useState({});
  const [selectedTourError, setSelectedTourError] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchTours = async () => {
      try {
        const toursRef = collection(db, "users");
        const querySnapshot = await getDocs(toursRef);

        const fetchedTours = [];
        const fetchedTourLocations = {};

        querySnapshot.forEach((doc) => {
          const userData = doc.data();
          if (userData.category === "Tours") {
            fetchedTours.push({ label: userData.username, value: userData.username });
            fetchedTourLocations[userData.username] = userData.address;
          }
        });

        setTours(fetchedTours);
        setTourLocations(fetchedTourLocations);
      } catch (error) {
        console.error("Error fetching tours:", error);
      }
    };

    // Call the fetcTours function
    fetchTours();
  ...
  const handleTourChange = (itemValue) => {
    setSelectedTour(itemValue);
    setPlace(tourLocations[itemValue]);
  };
  ...
  const handlebooking = async () => {
    ...
    if (!selectedTour || !place || !checkindate || !checkoutdate || !adults || !rooms) {
      // If any required field is empty, set error flags
      setSelectedTourError(!selectedTour);
      setPlaceError(!place);
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          <View style={styles.titleContainer}>
            ...
            <View style={styles.inputContainer}>
              ...
              >
                <Picker.Item label="Select tour" value="Tour" />
                {tours.map((tour) => (
                  <Picker.Item
                    label={tour.label}
                    value={tour.value}
...
export default NUToursBookingScreen;
