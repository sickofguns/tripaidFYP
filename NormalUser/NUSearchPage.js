...
const NUSearchUserScreen = () => {
  ...
  const renderUserItem = ({ item }) => (
    <View style={styles.userItem}>
      <TouchableOpacity
        onPress={() => navigateToUserProfile(item.username, item.type, item.verify)}
      >
      ...
      <TouchableOpacity onPress={() => handleFollowToggle(item.id)}>
        <Text
          style={[
            styles.followButton,
            { backgroundColor: item.isFollowing ? "#030D45" : "#FB7E3C" },
          ]}
        >
  ...
  const navigateToUserProfile = (userId, userType, verify) => {
...
export default NUSearchUserScreen;
